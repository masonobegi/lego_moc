 ...
 *
 * Color availability comes from the bundled OMR-derived data
 * (data/catalog/bundled-catalog.json), which is always present and is keyed by
 * LDraw color ids. An additional LDraw-native color source can be supplied
 ...
 *
 * A part/color combination that appears in NEITHER source is treated as
 * "not known to exist" and is never proposed. That biases the product towards
 * missing savings rather than recommending a color that was never produced,
...
export interface CatalogInputs {
  ...
  /**
   * Color availability from a source that uses LDRAW color ids.
   *
   * Deliberately not wired to the Rebrickable import: that data is keyed by
   * Rebrickable color ids, and treating them as LDraw ids would silently
   * corrupt every color-validity decision. The hook exists so a verified
   * LDraw-native color source can be dropped in without touching the
   * optimizer. See docs/RESEARCH.md section 10.
   */
  ldrawNativeColors?: {
    source: string;
    label: string;
    /** part id -> LDRAW color ids known to exist. */
    colorsByPart: ReadonlyMap<string, ReadonlySet<number>>;
    externalIds?: ExternalIdTable;
  };
  /** BrickLink color ids resolved by name against the live API, when available. */
...
export class DefaultCatalogService implements CatalogService {
  ...
  constructor(inputs: CatalogInputs) {
    ...
    if (!usingExtra) {
      limitations.push(
        `Color availability comes from ${this.bundled.setCount} official LEGO sets modeled in the ` +
          `LDraw Official Model Repository, covering ${this.bundled.partCount.toLocaleString()} parts. ` +
          `That is real data but far narrower than Rebrickable's full catalog, so many parts will ` +
          `have no cheaper color proposed simply because we cannot prove one exists.`,
      );
      limitations.push(
        'Run "npm run catalog:import" for Rebrickable\'s alternate-mold rules. Note that its ' +
          'color data uses Rebrickable color ids, so it is NOT used for color validity.',
      );
    }
    limitations.push(
      'A part/color combination that is not in the catalog is never proposed, even if the price ' +
        'data suggests it would be cheaper.',
    );

    this.status = {
      sourceId,
      label: usingExtra ? this.extraColors!.label : 'Bundled LDraw OMR catalog',
